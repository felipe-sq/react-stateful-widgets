...
export default function Programmers() {
  ...
  const [featuredId, setFeaturedId] = useState(listOfAwesome.id)
  ...
  const getNameOfFeatured = () => {
    ...
    setProgrammers(listOfAwesome.name)
    setFeaturedId(listOfAwesome.id)
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (getNameOfFeatured === true ? 'gold' : 'royalblue')// 🤔 color turns to gold, when celebrating */
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { dev.id/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
