...
export default function Programmers() {
  ...
  const getNameOfFeatured = (id) => {
    ...
    (id === programmers.id ? setFeaturedId('featured') : setFeaturedId(''))
    return
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (featuredId === true ? 'gold' : 'royalblue')// 🤔 color turns to gold, when celebrating */
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { dev.id/* in here set the featured id to be dev.id */, console.log(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
